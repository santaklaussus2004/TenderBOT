...
from llm import ChatGPTClient_nano,ChatGPTClient_luna
import re
import json
...
while True:
    ...
    for i,url in enumerate(base_urls):
        ...
        filename = Path("21.01.2026past.json")
        ...
        for page in range(1,1001):
            ...
            for link in links:
                ...
                trash = llm_nano.ask("ВОТ НАША КОМПАНИЯ  -  ТОО «ASTANA TECH-GROUP» занимается проектированием, строительством и комплексным оснащением спортивных, культурных и образовательных объектов под ключ. Компания поставляет и монтирует спортивные покрытия, паркет, искусственный газон, тартан, спортивное оборудование и тренажёры, трибуны и кресла, мультимедийное, звуковое и слаботочное оборудование. Также выполняет устройство футбольных и спортивных площадок, ледовых арен, воздухоопорных сооружений, ограждений, МАФ и благоустройство территорий, а также оснащает учебные и актовые залы.Посмотри по нашей ли теме этот лот или лоты, и ответь одним словом - да или нет,твой лимит 1 слово ВСЕГДА. Если из всех лотов выигранных хотя бы один совпадает с нашим, то ответ да.", json.dumps(tender["winners"], ensure_ascii=False)).lower()
                ...
                if trash == "нет":
                    ...
                    trash = True 
                    print(f"Мусор - {link}")
                    trash_filename = Path("trash.json")
                    if not trash_filename.exists():
                        with trash_filename.open("w", encoding="utf-8") as file:
                            json.dump([], file, ensure_ascii=False, indent=2)
                    with open(trash_filename,'r',encoding='utf8') as file:
                    ...
                    if already == False:
                        ...
                        with open(trash_filename,'w',encoding='utf8') as file:
                    ...
                    if lot_winner:
                        ...
                        if daddy_yes == True:
                            ...
                            if winner_data["phone_number"] != None or winner_data["email"] != None:
                                daddy_yes = True
                                message = llm_luna.ask("""Твоя задача писать сообщения рекламу нашей компании основываясь на инфе о нашей компании, и об инфе о выигранном тендере, и об инфе о победителях тендера. Сообщение адресовано победителям. Если тендер закончился давно, то просто предложи им наши услуги в будущих их начинаниях. Если недавно, спроси нужно ли им еще что-то. Наша компания:ASTANA TECH-GROUP
    ...
    #                         tender["winners"].append(winner_data)
        ...
        """
                            if not company_html.strip():
                                ...
                                result_text = llm_nano.ask(prompt, company_html)
